...
* ``ss=3.76/-20.1`` represents two types of signal strength.
  ...
  (RSSI). It also has no useful units, and is included solely for those
  ...
  (which is basically the usual formula of 10 * log10(power) plus some
  dump978-fa strangeness).
  This is a negative number that gets closer to zero with a stronger signal,
  and further away with a higher signal. If your SDR I/Q values are
  calibrated, this could provide useful results for comparison. Like the
  first number, this number is calculated using the power from the sync word.
